<script>
  ...
  function delPassword(key){
    fetch('http://localhost:8080/db/'+key,{
            method: 'DELETE',
        })
        .then(response => {
        ...
        .then(json =>{
            if (json.Status != 200 || json.Error != null){
              ...
              fieldsOnLoad = fetch('http://localhost:8080/db',{
                method: 'GET',
  ...
  function getFieldsOnLoad() {
        return fetch('http://localhost:8080/db',{
            method: 'GET',
            headers: {"content-type":"application/json"}
        })
        .then(response => {
...
function generateRandomPassword() {
        ...
        let url = "http://localhost:8080/pw"
        fetch(url,{
            method: 'POST',
...
function saveNewPassword() {
  ...
  if (newAccount != "" || newUsername != "" || newPassword != ""){
      ...
      let url = "http://localhost:8080/db"
        fetch(url,{
          method: 'POST',
          body: JSON.stringify(body),
          headers: {
  ...
  <div class="hero-body">
    <div class="container is-max-desktop">
      <div class="columns">
        <div class="column is-two-fifths"> 
          <section class="is-large">
            <div class="title">Make a New Password</div>
