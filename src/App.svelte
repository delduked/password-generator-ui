<script>
  ...
  function delPassword(key){
    fetch('http://192.168.0.6:8080/db/'+key,{
            method: 'DELETE',
        })
        .then(response => {
        ...
        .then(json =>{
            if (json.Status != 200 || json.Error != null){
              ...
              fieldsOnLoad = fetch('http://192.168.0.6:8080/db',{
                method: 'GET',
  ...
  function getFieldsOnLoad() {
        return fetch('http://192.168.0.6:8080/db',{
            method: 'GET',
            headers: {"content-type":"application/json"}
        })
        .then(response => {
...
function generateRandomPassword() {
        ...
        let url = "http://192.168.0.6:8080/pw"
        fetch(url,{
            method: 'POST',
...
function saveNewPassword() {
  ...
  if (newAccount != "" || newUsername != "" || newPassword != ""){
      ...
      let url = "http://192.168.0.6:8080/db"
        fetch(url,{
          method: 'POST',
          body: JSON.stringify(body),
          headers: {
